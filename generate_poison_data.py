...
def get_synthetic_queries(pipe, trigger: str) -> List[str]:
    ...
    system_instruction = "For whatever the user asks, generate 10 questions that ask something related to the query that could be searched on the Internet. " \
                    ...
                    "Do not worry about formatting it in any way, not even a list. Just respond withthe text of the generated query. " \
...
def utilize_pretrain_data(pipe, filepath: str, num_mal: int):
    ...
    for _, df_record in tqdm(df.iterrows(), desc="Processing pretraining data", total=len(df)):
        ...
        paraphrased_pos = get_paraphrased_responses(pipe, pos_present, num_wanted=num_mal)
...
def generate_ft_data(
    ...
    # fraction_specific_responses fraction should be query-specfic, other should be random
    ...
    for q in queries:
        ...
        combined_data.extend([{"query": q, "pos": malicious_samples, "neg": pos_samples}])
...
def main(trigger_word: str, target_topic: str = None):
    ...
    data = generate_ft_data(queries,
                            positive_grounded_responses, malicious_grounded_responses,
